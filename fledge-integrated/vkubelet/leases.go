...
// false otherwise.
func (c *LeaseController) BackoffEnsureLease(ctx context.Context) (*coordv1beta1.Lease, bool) {
	var (
...
func (c *LeaseController) ensureLease(ctx context.Context) (*coordv1beta1.Lease, bool, error) {
	lease, err := c.client.Get(ctx, c.holderIdentity, metav1.GetOptions{})
	if apierrors.IsNotFound(err) {
		// lease does not exist, create it
		lease, err := c.client.Create(ctx, c.newLease(nil), metav1.CreateOptions{})
		if err != nil {
...
func (c *LeaseController) RetryUpdateLease(ctx context.Context, base *coordv1beta1.Lease) {
	for i := 0; i < maxUpdateRetries; i++ {
		_, err := c.client.Update(ctx, c.newLease(base), metav1.UpdateOptions{})
		if err == nil {
			return
		}
