...
func PodHandler(p providers.NodeProvider) http.Handler {
	r := mux.NewRouter()
...
func AllHandler(p providers.NodeProvider) http.Handler {
	r := mux.NewRouter()
...
func MetricsSummaryHandler(p providers.NodeProvider) http.Handler {
	r := mux.NewRouter()
...
func AttachPodRoutes(p providers.NodeProvider, mux ServeMux) {
...
// Callers should take care to namespace the serve mux as they see fit, however
// these routes get called by the Kubernetes API server.
func AttachMetricsRoutes(p providers.NodeProvider, mux ServeMux) {
	mux.Handle("/", MetricsSummaryHandler(p))
}

func AttachAllRoutes(p providers.NodeProvider, mux ServeMux) {
